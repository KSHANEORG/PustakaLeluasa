<div style="max-width: 1100px; margin: 0 auto; padding: 2rem;">
    ...
    <div style="display: grid; grid-template-columns: 320px 1fr; gap: 2rem;">
        ...
        <div>
            ...
            <a href="{{ "https://wa.me/6285379426392?text=I'm%20interested%20in%20your%20BOOK%20for%20sale%20".$book->bookname }}" target="_blank">Hubungi</a>

            @auth
                <form method="POST" action="{{ route('collection.add', ['specialbookid' => $book->specialbookid]) }}" style="margin-top: 0.75rem;">
                    @csrf
                    <button type="submit">Tambah ke Koleksi</button>
                </form>
            @else
                <p><a href="{{ url('/app/login') }}">Login untuk tambah ke koleksi</a></p>
            @endauth
        </div>
    </div>
</div>
